    ...
    def get_all_samples(self):
        for i in range(len(self.data_dir)):
            ...
            if self.is_training:
                ...
                elif "CTW" in self.data_dir[i] or 'tpa' in self.data_dir[i] or 'syn_curve' in self.data_dir[i] or 'coco' in self.data_dir[i]:
                    image_path=[self.data_dir[i]+'/train_images/'+timg.strip() for timg in image_list]
                ...
                elif 'CTW1500' in self.data_list[i] or 'tpa' in self.data_list[i] or 'coco' in self.data_dir[i]:
    ...
    def load_ann(self):
        ...
        for  gt in tqdm(self.gt_paths, total=lens):
            ...
            for line in reader:
                ...
                #     line = [i.strip('\ufeff').strip('\xef\xbb\xbf') for i in parts]
                #     poly = np.array(list(map(float, line[:8]))).reshape((-1, 2)).tolist()
                elif 'MPSC' in self.data_dir[0] or 'coco' in self.data_dir[0]:
                    line = [i.strip('\ufeff').strip('\xef\xbb\xbf') for i in parts]
                    try:
                        if line==['']:
                            continue
    ...
    def get_ann_syn_curve(self,img,index,image_path):
        ...
        for box in (bboxes):
            ...
            try:
                ...
                print(gtt)
                gtt = np.array(list(map(float, gtt))).reshape((-1, 2)).tolist()
                print(gtt,22)
            except:
                # print(box,11)
                ...
                #     box = box[:-1]
                # print(box,22)
                line = [i.strip('\ufeff').strip('\xef\xbb\xbf') for i in box]
                num_points = math.floor((len(line) - 1) / 2) * 2
                gtt = np.array(list(map(float, line[:num_points]))).reshape((-1, 2)).tolist()

                print(gtt,333)
            # except:
            #     print(gtt,222,len(gtt))
            # print(gtt)
            z={'poly':gtt, 'text':'qzz'}
            lines.append(z)
        return lines
